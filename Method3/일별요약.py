...
# 1. 설정
# 파일 경로와 파일 이름
file_path = "C:/Users/82109/Desktop/벼리/7학기/텍스트마이닝/GPT기반방법론/날짜별_요약_통합.csv" # 경로 변경하기

# 결과물을 저장할 폴더 설정 (원본 파일 위치에 '샘플 이벤트' 폴더 생성)
folder_path = os.path.dirname(file_path)
save_dir = os.path.join(folder_path, "논문일별요약")
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
save_path_csv = os.path.join(save_dir, f"일별요약.csv") # 파일명 변경하기
...
# 2. 프롬프트 정의
prompt_header = """
You are given a collection of paper titles and abstracts published on the same date.
Please analyze these papers together to extract the main research trends for that date.

**Output Requirements**
1. Identify shared research themes or emerging directions among the papers.
2. Summarize in 3–5 sentences the overall focus of research for that date.
3. Be objective and factual — avoid speculation.
4. Emphasize recurring keywords, methods, or target applications (e.g., performance optimization, motion analysis, injury prevention).
5. Output format:
   - **Date:** (insert date)
   - **Key Themes:** (concise summary paragraph)
...
try:
    if not all(col in df.columns for col in ['date', 'n_papers', 'merged_summaries']):
        print(f"'{os.path.basename(file_path)}' 파일에 필수 컬럼(merged_summaries, n_papers, date)이 없어 건너뜁니다.")
        exit()
    ...
    for idx, row in df.iterrows():
        ...
        article_text = row.get('merged_summaries', 'N/A') 
        ...
        new_row = {
                "date": row.get("date", ""),
                "merged_summaries": row.get("merged_summaries", ""),
                "요약": event
            }
